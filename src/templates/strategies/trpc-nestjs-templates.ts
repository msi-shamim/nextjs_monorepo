...
export class TrpcNestjsTemplateStrategy implements ApiStyleStrategy {
...
/** Mount tRPC as Express middleware inside NestJS */
...
`,
  ...
  clientFiles(config: ProjectConfig): Record<string, string> {
    return trpcClientFiles(config);
  }

  setupInstructions(_config: ProjectConfig): string {
    return `// tRPC (NestJS):
// In apps/api/src/main.ts, after app creation:
...
/** Shared tRPC client files for Next.js (used by both NestJS and Express adapters) */
function trpcClientFiles(config: ProjectConfig): Record<string, string> {
  return {
